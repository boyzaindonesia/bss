<div class="the-box no-border">
    <div class="btn-toolbar toolbar-btn-action">
        <span class="magnific-popup-ajax">
            <a href="#form-produk" class="btn btn-primary margin-bottom" data-effect="mfp-zoom-in" data-ajax-url="<?php echo $own_links.'/get_table_add_products/'.$val->product_group_id?>">Tambah Produk</a>
        </span>
    ...
    <div class="table-responsive">
        <table class="table table-th-block table-dark datatable">
            ...
            <?php
            $product_group = get_product_by_group($val->product_group_id);
            if(count($product_group) > 0){
                $i = 1;
                foreach($product_group as $r){ ?>
                <tr>
                    ...
                    <td class="nobr"><?php echo get_product_category_name($r->product_category_id);?></td>
                    <td class="nobr text-center">
                        <a href="<?php echo $own_links.'/delete_detail/'.$val->product_group_id.'-'.$r->product_id.'-'.changeEnUrl($r->product_name);?>" onclick="return confirm('Are you sure delete ?');" class="btn btn-danger btn-xs" data-toggle="tooltip" data-original-title="Hapus"><i class="fa fa-times"></i></a>
                    </td>
                </tr>
                <?php
                    $i += 1;
        ...
        <form id="form1" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="btn-toolbar toolbar-btn-action">
                <div class="btn-group pull-right">
                    <div class="btn btn-info btn-save-multi" data-url=""><i class="fa fa-plus"></i> Tambahkan ( 0 )</div>
                </div>
            </div>
            <div class="table-responsive">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-save-multi', function(e){
            ...
            if(checked_files.length == 0){
                ...
                var thisIdGroup = '<?php echo $val->product_group_id ?>';
                $.ajax({
                    type: 'POST',
                    url: OWN_LINKS+'/save_multi_detail',
                    data: {'thisId':thisId,'thisIdGroup':thisIdGroup,'thisAction':'save'},
                    async: false,
                    cache: false,
                    dataType: 'json',
