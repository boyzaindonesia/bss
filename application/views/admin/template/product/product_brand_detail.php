<div class="the-box no-border">
    <div class="btn-toolbar toolbar-btn-action">
        <span class="magnific-popup-ajax">
            <a href="#form-produk" class="btn btn-primary margin-bottom" data-effect="mfp-zoom-in" data-ajax-url="<?php echo $own_links.'/get_table_add_products/'.$val->product_brand_id?>">Tambah Produk</a>
        </span>
    ...
    <div class="table-responsive">
        <table class="table table-th-block table-dark datatable">
            ...
            <?php
            $product_brand = get_list_product_brand($val->product_brand_id);
            if(count($product_brand) > 0){
                ...
                <tr>
                    ...
                    <td class="nobr text-center">
                        <a href="<?php echo $own_links.'/delete_detail/'.$val->product_brand_id.'-'.$r->product_id.'-'.changeEnUrl($r->product_name);?>" onclick="return confirm('Are you sure delete ?');" class="btn btn-danger btn-xs" data-toggle="tooltip" data-original-title="Hapus"><i class="fa fa-times"></i></a>
                    </td>
                </tr>
                <?php
                    $i += 1;
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-save-multi', function(e){
            ...
            if(checked_files.length == 0){
                ...
                var thisIdBrand = '<?php echo $val->product_brand_id ?>';
                $.ajax({
                    type: 'POST',
                    url: OWN_LINKS+'/save_multi_detail',
                    data: {'thisId':thisId,'thisIdBrand':thisIdBrand,'thisAction':'save'},
                    async: false,
                    cache: false,
                    dataType: 'json',
