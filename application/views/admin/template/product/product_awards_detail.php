<ul class="nav nav-tabs item-color">
    <li><a href="<?php echo $own_links ?>">Master</a></li>
    <?php
    $get_product_awards = get_product_awards();
    foreach ($get_product_awards as $k => $v) {
    ?>
    <li class="<?php echo ($val->product_awards_id==$v->product_awards_id?'active':'') ?>"><a href="<?php echo $own_links.'/detail/'.$v->product_awards_id.'-'.changeEnUrl($v->product_awards_name)?>"><?php echo $v->product_awards_name ?></a></li>
    <?php } ?>
</ul>
...
<div class="the-box no-border">
    <div class="btn-toolbar toolbar-btn-action">
        <span class="magnific-popup-ajax">
            <a href="#form-produk" class="btn btn-primary margin-bottom" data-effect="mfp-zoom-in" data-ajax-url="<?php echo $own_links.'/get_table_add_products/'.$val->product_awards_id?>">Tambah Produk</a>
        </span>
    ...
    <div class="table-responsive">
        <table class="table table-th-block table-dark datatable">
            ...
            <?php
            $product_awards = get_list_product_awards($val->product_awards_id);
            if(count($product_awards) > 0){
                $i = 1;
                foreach($product_awards as $r){ ?>
                <tr>
                    ...
                    <td class="nobr"><?php echo get_name_product_status($r->product_status_id);?></td>
                    <td class="nobr text-center">
                        <a href="<?php echo base_url().'admin/product/edit/'.$r->product_id.'-'.changeEnUrl($r->product_name);?>?next=<?php echo $own_links.'/detail/'.$val->product_awards_id.'-'.changeEnUrl(get_product_awards($val->product_awards_id)->product_awards_name);?>" class="btn btn-info btn-xs" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil"></i></a>

                        <a href="<?php echo $own_links.'/delete_detail/'.$val->product_awards_id.'-'.$r->product_id.'-'.changeEnUrl($r->product_name);?>" onclick="return confirm('Are you sure delete ?');" class="btn btn-danger btn-xs" data-toggle="tooltip" data-original-title="Hapus"><i class="fa fa-times"></i></a>
                    </td>
                </tr>
                <?php
                    $i += 1;
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-save-multi', function(e){
            ...
            if(checked_files.length == 0){
                ...
                var thisIdAwards = '<?php echo $val->product_awards_id ?>';
                $.ajax({
                    type: 'POST',
                    url: OWN_LINKS+'/save_multi_detail',
                    data: {'thisId':thisId,'thisIdAwards':thisIdAwards,'thisAction':'save'},
                    async: false,
                    cache: false,
                    dataType: 'json',
