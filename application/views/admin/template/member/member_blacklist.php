                            ...
                            <form action="<?php echo !empty($url_form)?$url_form:$own_links;?>" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                                ...
                                <div class="btn-group">
                                    ...
                                    <select name="order_by" class="form-control" onchange="this.form.submit()">
                                        <option value="mt_member_blacklist.member_blacklist_date-desc" <?php echo ($order_by=='mt_member_blacklist.member_blacklist_date'&&$order_dir=='desc'?'selected':'') ?> >Date: New to Old</option>
                                        <option value="mt_member_blacklist.member_blacklist_date-asc" <?php echo ($order_by=='mt_member_blacklist.member_blacklist_date'&&$order_dir=='asc'?'selected':'') ?> >Date: Old to New</option>
        ...
        <table class="table table-th-block table-dark">
            ...
            <thead>
                <tr>
                    ...
                    <th>Pesan</th>
                    <th class="nobr text-center">Type Member</th>
                    <th class="nobr text-center">Status Member</th>
            ...
            if(count($data) > 0){
                ...
                <tr>
                    ...
                    <td class="nobr"><?php echo $r->member_blacklist_email;?></td>
                    <td><?php echo getFirstParaSm($r->member_blacklist_desc);?></td>
                    <td class="nobr text-center">
                        <span class="label <?php echo isset($r->member_id)&&$r->member_id!='0'?'label-success':'label-default' ?>"><?php echo isset($r->member_id)&&$r->member_id!='0'?'Member':'Non Member' ?></span>
                    </td>
                    <td class="nobr text-center">
                        <span class="label <?php echo isset($r->member_id)&&$r->member_id!='0'?get_status_member($r->member_blacklist_status)['class']:'' ?>"><?php echo isset($r->member_id)&&$r->member_id!='0'?get_status_member($r->member_blacklist_status)['status']:'' ?></span>
                    </td>
                    <td class="nobr text-right"><span class="label label-default"><?php echo convDateTimeTable($r->member_blacklist_date) ?></span></td>
                    <td class="nobr">
                        <?php link_action($links_table_item,$r->member_blacklist_id,changeEnUrl($r->member_blacklist_email));?>
                    </td>
                </tr>
                <?php
                    $i += 1;
