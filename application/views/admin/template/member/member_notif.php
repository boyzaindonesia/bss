                            ...
                            <form action="<?php echo !empty($url_form)?$url_form:$own_links;?>" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                                ...
                                <div class="btn-group">
                                    ...
                                    <select name="order_by" class="form-control" onchange="this.form.submit()">
                                        <option value="mt_member_notif.member_notif_date-desc" <?php echo ($order_by=='mt_member_notif.member_notif_date'&&$order_dir=='desc'?'selected':'') ?> >Date: New to Old</option>
                                        <option value="mt_member_notif.member_notif_date-asc" <?php echo ($order_by=='mt_member_notif.member_notif_date'&&$order_dir=='asc'?'selected':'') ?> >Date: Old to New</option>
        ...
        <table class="table table-th-block table-dark">
            ...
            <thead>
                <tr>
                    ...
                    <th class="nobr">Nama Member</th>
                    <th>Email</th>
                    <th>Pesan</th>
                    <th class="nobr text-center">Berulang</th>
                    <th class="nobr text-center">Keluarkan</th>
            ...
            if(count($data) > 0){
                ...
                <tr>
                    ...
                    <td class="nobr"><?php echo get_member_name($r->member_id);?></td>
                    <td><?php echo get_member_email($r->member_id);?></td>
                    <td><?php echo getFirstParaSm($r->member_notif_desc);?></td>
                    <td class="nobr text-center">
                        <span class="label <?php echo isset($r->member_notif_repeat)?get_status_default($r->member_notif_repeat)['class']:'' ?>"><?php echo isset($r->member_notif_repeat)?get_status_default($r->member_notif_repeat)['status']:'' ?></span>
                    </td>
                    <td class="nobr text-center">
                        <span class="label <?php echo isset($r->member_notif_logout)?get_status_default($r->member_notif_logout)['class']:'' ?>"><?php echo isset($r->member_notif_logout)?get_status_default($r->member_notif_logout)['status']:'' ?></span>
                    </td>
                    <td class="nobr text-right"><span class="label label-default"><?php echo convDateTimeTable($r->member_notif_date) ?></span></td>
                    <td class="nobr text-center">
                        <div class="onoffswitch">
                            <input type="checkbox" name="switch_sidebar_<?php echo $r->member_notif_id ?>" class="onoffswitch-checkbox" id="switch_sidebar_<?php echo $r->member_notif_id ?>" onclick="changeStatus(this,'<?php echo $own_links.'/change_status/'.$r->member_notif_id;?>')" value="1" <?php if($r->member_notif_status == "1"){ echo 'checked'; } ?> >
                            <label class="onoffswitch-label" for="switch_sidebar_<?php echo $r->member_notif_id ?>">
                    ...
                    <td class="nobr">
                        <?php link_action($links_table_item,$r->member_notif_id,changeEnUrl(get_member_name($r->member_id)));?>
                    </td>
                </tr>
                <?php
                    $i += 1;
                ...
                echo '<tr><td colspan="9">Tidak ditemukan di database.</td></tr>';
            }
            ?>
            </tbody>
        </table>
