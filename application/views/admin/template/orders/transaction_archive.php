...
<div class="the-box no-border">
                            ...
                            <form action="<?php echo !empty($url_form)?$url_form:$own_links;?>" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                                ...
                                <div class="btn-group">
                                    ...
                                    <select name="order_by" class="form-control" onchange="this.form.submit()">
                                        <option value="mt_orders_archive.orders_date-desc" <?php echo ($order_by=='mt_orders_archive.orders_date'&&$order_dir=='desc'?'selected':'') ?> >Date: New to Old</option>
                                        <option value="mt_orders_archive.orders_date-asc" <?php echo ($order_by=='mt_orders_archive.orders_date'&&$order_dir=='asc'?'selected':'') ?> >Date: Old to New</option>
                                    </select>
                                </div>
                            </form>
                        </div>
    ...
    <div class="relative no-border mt-20">
        <div class="cart-list mb-0">
            ...
            if(count($data) > 0){
                foreach($data as $r){
                    $payment = get_detail_orders_payment_archive($r->orders_id);
            ?>
                ...
                <div class="cart-list-content">
                    ...
                    <div class="btn-group pull-right" style="position: absolute; top: 5px; right: 5px;">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li><a href="javascript:void(0);" class="btn-delete" data-id="<?php echo $r->orders_id;?>" data-status="cancel">Hapus Pesanan</a></li>
                        </ul>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-view-product', function(e){
            ...
            if($thisVal != ''){
                setTimeout(function(){
                    $.ajax({
                        type: 'POST',
                        url: MOD_URL+'admin/transaction_detail_product/form_detail_product_archive',
        ...
        $(document).on('click', '.btn-delete', function(e){
            ...
            if($thisId != ''){
                var swalText = "Yakin ingin menghapus pesanan!";
                swal({
                    title: 'Are you sure?',
                    text: swalText,
                    type: 'warning',
                    ...
                    $.ajax({
                        type: 'POST',
                        url: MOD_URL+'admin/transaction_process/deleted_orders',
