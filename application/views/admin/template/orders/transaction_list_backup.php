...
<div class="the-box no-border">
                            ...
                            <form action="<?php echo !empty($url_form)?$url_form:$own_links;?>" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                                ...
                                <div class="btn-group">
                                    ...
                                    <select name="order_by" class="form-control" onchange="this.form.submit()">
                                        <option value="mt_orders_bk.orders_date-desc" <?php echo ($order_by=='mt_orders_bk.orders_date'&&$order_dir=='desc'?'selected':'') ?> >Date: New to Old</option>
                                        <option value="mt_orders_bk.orders_date-asc" <?php echo ($order_by=='mt_orders_bk.orders_date'&&$order_dir=='asc'?'selected':'') ?> >Date: Old to New</option>
                                    </select>
                                </div>
                            </form>
                        </div>
    ...
    <div class="relative no-border mt-20">
        <div class="cart-list mb-0">
            ...
            if(count($data) > 0){
                foreach($data as $r){
                    $payment = get_detail_orders_payment_bk($r->orders_id);
            ?>
        ...
        $(document).on('click', '.btn-view-product', function(e){
            ...
            if($thisVal != ''){
                setTimeout(function(){
                    $.ajax({
                        type: 'POST',
                        url: MOD_URL+'admin/transaction_detail_product/form_detail_product_backup',
