...
<div class="the-box no-border">
    ...
    <h3 class="panel-title text-uppercase mb-10"><a href="<?php echo base_url() ?>admin/report/product" target="_blank">Data Penjualan</a></h3>
        ...
        function getChartOrders(){
            ...
            if(dateStartReport != "" && dateEndReport != ""){
                setTimeout(function () {
                    $.ajax({
                        type: 'POST',
                        url: MOD_URL+'admin/transaction_report/ajax_get_orders_report',
        ...
        <table class="table table-th-block table-dark datatable-all-custom-sort table-font-small">
            ...
            if(count($data) > 0){
                foreach($data as $r){
                    ...
                    $checked_files = "";
                    if($r->orders_shipping_resi != '' && $r->orders_print != 1){
                        $total_resi += 1;
                        $checked_files = 'checked';
                    }

                    $get_orders_detail = get_orders_detail($r->orders_id);
                    if(count($get_orders_detail) > 0){
                        $checked_files = 'checked';
                        $tr_class = 'info';
                    }
                ...
                <tr class="<?php echo $tr_class ?>" data-id="<?php echo $r->orders_id;?>" title="<?php echo 'Nama Pengirim: '.$orders_ship_name ?>">
                    ...
                    <td class="nobr">
                        ...
                        <div class="price_product"><?php echo ($r->orders_source_price==$r->orders_price_product?'':'Harga MP: '.convertRp($r->orders_source_price)) ?></div>
                    </td>
                    <td>
                    ...
                    <td class="nobr">
                        <button type="button" class="btn btn-warning btn-xs btn-add-product" data-id="<?php echo $r->orders_id;?>" data-toggle="tooltip" data-original-title="Input Produk"><i class="fa fa-send"></i> (<?php echo count($get_orders_detail); ?>) Produk</button>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.form_save_add_detail_product .popup-close', function(e){
            e.preventDefault();
            window.location.reload(true);
        });
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('submit', 'form.form_save_form_label', save_form_label );

        function save_form_label(e){
            if (typeof e !== 'undefined') e.preventDefault();
            var $this = $(this);
            var form = $this;
