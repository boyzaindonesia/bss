            ...
            <div class="list-group success square no-side-border">
                ...
                <?php } ?>
                    ...
                    if(count($data) > 0){
                        $i = 1;
                        foreach($data as $r){ ?>
                        <a href="<?php echo $own_links.'/view/'.$r->message_id.'-'.changeEnUrl($r->message_name);?>" class="list-group-item mail-list <?php echo ($r->notify!='0'?'unread':'') ?>">
                            <?php if(check_action($links_table_item,'delete')){ ?>
                            <input type="checkbox" name="filesToDel[]" value="<?php echo $r->message_id ?>">
                            <?php } ?>
                            <span class="avatar avatar-mail <?php echo getAvatarMail($r->message_name) ?>"><?php echo getCharMail($r->message_name) ?></span>
                            <span class="name"><?php echo ucwords($r->message_name) ?></span>
                            <span class="subject"><?php echo getFirstParaSm($r->message_subject) ?></span>
                            <span class="time"><?php echo convDateTimeTable($r->message_date) ?></span>
                        </a>
                        <?php
                            $i += 1;
                        } 
