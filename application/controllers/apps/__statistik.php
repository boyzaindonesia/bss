...
class statistik extends FrontController {
	var $cur_menu = '';
	...
	// 			$rows['statistik']  = $this->data_table;
	// 		}
	// 	}
	...
	function statistik_orders(){
	    ...
	    if (isset($postdata)) {
	    	$request 	= json_decode($postdata);
			$thisAction = mysql_real_escape_string($request->thisAction);
			if($thisAction == 'getdatapenjualan'){
				$par_filter = array();
				foreach ($request as $key => $value) {
					$par_filter[$key] = $value;
				}
				$this->data_table = $this->M->data_statistik_penjualan($par_filter);
				$rows['statistik_orders']  = $this->data_table;
			}
		}
