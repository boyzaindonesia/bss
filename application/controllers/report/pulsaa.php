...
class Pulsaa extends AdminController {  
	function __construct()    
	{
		parent::__construct();    
		$this->_set_action();
		$this->_set_title( 'List Transaksi Pulsa' );
		$this->folder_view = "report/";
		$this->prefix_view = strtolower($this->_getClass());
		$this->load->model("mdl_order_ppl","M");
		$this->breadcrumb[] = array(
				"title"		=> "Data Transaksi Pulsa",
				"url"		=> $this->own_link
			);
	}
	...
	//debugCode($this->own_link);
		...
		$this->data_table = $this->M->data_article_pulsa($par_filter);
		...
		//debugCode($_POST);	
		$data['url'] = base_url()."report/pulsa/index";
