<?php
class mdl_scraper extends CI_Model{ 
	
	var $tabel = 'mt_scraper';
	
	function __construct(){
		parent::__construct();
	} 

	function data_scraper($p=array(),$count=FALSE){
		$total = 0;
		/* table conditions */

		$this->db->select('mt_scraper.*');
		$this->db->where("scraper_istrash",0);

		/* where or like conditions */
		if( trim($this->jCfg['search']['date_end']) != "" ){
			//debugCode($this->jCfg['search']['date_end']);
			$this->db->where("( scraper_date <= '".$this->jCfg['search']['date_end']." 23:59:59' )");
		}
		
		if( trim($this->jCfg['search']['date_start'])!="" ){
			//debugCode($this->jCfg['search']['date_end']);
			$this->db->where("( scraper_date >= '".$this->jCfg['search']['date_start']." 00:00:00' )");
		}
		// dont modified....
		...
		if(trim($this->jCfg['search']['order_by'])!=""){
			...
			$this->db->order_by('mt_scraper.scraper_date','desc');
		}
		
		$qry = $this->db->get('mt_scraper');
		if($count==FALSE){
			$total = $this->data_scraper($p,TRUE);
			return array(
					"data"	=> $qry->result(),
					"total" => $total
				);
