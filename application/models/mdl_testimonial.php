...
class mdl_testimonial extends CI_Model{ 
	...
	var $tabel = 'mt_testimonial';
	
	function __construct(){
		parent::__construct();
	} 

	function data_testimonial($p=array(),$count=FALSE){
		$total = 0;
		/* table conditions */
		
		$this->db->select('*');
		$this->db->where("testimonial_istrash",0);
		...
		$qry = $this->db->get('mt_testimonial');
		if($count==FALSE){
			$total = $this->data_testimonial($p,TRUE);
			return array(
					"data"	=> $qry->result(),
					"total" => $total
				);
